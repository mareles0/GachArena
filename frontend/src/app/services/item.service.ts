...
export class ItemService {
  ...
  async getItemById(itemId: string): Promise<Item | null> {
    const data = await this.http.get(`${environment.backendUrl}/items/${itemId}`).toPromise() as any;
    return data ? { id: itemId, ...data } as Item : null;
  }
  ...
  async getUserItems(userId: string): Promise<UserItem[]> {
    const data = await this.http.get(`${environment.backendUrl}/userItems/user/${userId}`).toPromise() as any[];
